...
The user's 2026-09-06 balance and crash-prevention requests are preserved in [FORTIFICATIONS_TODO.md](FORTIFICATIONS_TODO.md), separate from the existing War Day queue. Read that file before working on those requests and present an item-specific feasibility write-up before implementation.

Active balance request: **Requested changes — “Ban `reliquified_irons_spells_and_spellbooks:sinner_crown` for crashing.”** The exact 0.2.7 registry ID is now in the central banned-item registry. Existing inventory, equipment, pickup, use, and dropped-item removal applies; Curios equip attempts are additionally rejected before activation, and highest-priority cleanup now runs every player tick to remove already-equipped crash-sensitive items immediately after login. `BannedItemRegistryTest` and the full build pass. Runtime crash-safety verification remains in `FORTIFICATIONS_TODO.md`, so the item is still unchecked.

Active War Day request: **Arena preparation — “Add `/warday legal` as a separate legality check that verifies the nexus can be reached without breaking blocks from all four corners of the owning team's connected claimed-chunk cluster, detecting nexus rooms filled with layers of blocks.”** `/warday legal`, `/warday legal status`, and `/warday legal cancel` are implemented as an independent tick-budgeted read-only job. It resolves four surface ingresses and runs bounded player-space searches inside the nexus-connected claim, reporting per-corner and overall results without changing validation, scanning, preparation eligibility, or saved match state. Calculation tests and the full build pass; the live FTB-claim and block-shape matrix remains required, so the canonical item stays unchecked.
...
- The NeoForge 1.21.1 Voxy project now builds against the pack's Sodium 0.8.12 dependency. `build.gradle` extracts Sodium's nested implementation JAR for compile-time use and excludes the obsolete pre-0.8 `VoxySodiumOptions` integration; `ShaderLoader` consumes Sodium 0.8's `ParsedShader.src()` result. The first runtime attempt failed when `MixinRenderSectionManager` used Sodium 0.6's three-argument constructor descriptor. Log `latest (1).log` confirmed Sodium 0.8.12 expects `(ClientLevel, int, SortBehavior, CommandList)`. That constructor injection and both `MixinDefaultChunkRenderer.render` injections now include Sodium 0.8's added arguments; the other enabled Sodium mixin fields, method descriptors, redirect targets, and invoked bytecode targets were audited against the resolved 0.8.12 implementation JAR. `gradlew.bat build --no-problems-report` succeeded on 2026-08-24, and mixin packaging validation passed for all 19 declared mixins. The replacement `voxy-0.2.9-alpha.jar` in `voxy mod folder` matches the build artifact at 89,109,990 bytes with SHA-256 `ED9711FD3CA5913B70F112A26C87E0D9E982A32D47E6D6C6645A3DE984CC31D7`. A new Minecraft launch/world-entry test remains required.
...
- The Warday mod has a command surface under `/warday` in `WarDayCommands`:
  ...
  - `/warday legal` independently checks whether a player-sized route reaches a nexus-adjacent standing position from surface ingresses nearest all four corners of the defender's nexus-connected claim. `/warday legal status` reports phase/progress and `/warday legal cancel` stops the read-only job.
...
- Classes background: the supplied `C:/Users/lylec/Downloads/2026-09-06_14.44.40.png` is bundled unchanged as `assets/fortifications/textures/gui/classes_background.png` (1920x1080, SHA-256 `9474ED224F33FE107355710112F826B6D21E5CDF10E71CF8AE4F9F747CDC1F01`). The separate `fortifications_classes-template/data/fortifications_classes/puffish_skills/categories/example/category.json` now references that texture with matching dimensions and the existing `fill` position. Deployment requires the updated category JSON in the active Classes datapack plus the refreshed mod on clients; the Classes datapack is still separate from the JAR. Manual verification remains: reload the updated datapack/restart the client, open Classes, and check image loading, crop/proportions at different GUI scales, and node/text readability. No skill allocations or gameplay rules changed.
- The latest merged `.\gradlew.bat clean build --no-problems-report` succeeded outside the filesystem sandbox on 2026-09-06 after sandboxed NeoForge artifact generation repeatedly hit Windows `AccessDeniedException`. All 19 tasks passed, including `WarDayLegalitySearchTest` and every existing focused check. The artifact and refreshed `MYTH MODS FOR DEREK/fortifications-1.0.0.jar` match at 4,171,991 bytes with SHA-256 `F29D625BE521E8E3CAE5E87CB1CD3B41FA64147D30ED63485024F2A86C6DD2D5`; the bundle contains only the combined JAR. Static inspection confirmed the legality job/search classes are packaged.
...
- JourneyMap 6.0.x/API 2.0.0 exposes a server-side per-receiver/per-target `PlayerRadarUpdateEvent`. Warday subscribes when JourneyMap is loaded and preserves visibility only when both players resolve to the same FTB Team ID. It never makes a JourneyMap-hidden player visible, applies globally including operators, and fails closed when team state is unavailable. The user's refreshed reference list removed the standalone compile API and currently has JourneyMap 6.0.4 disabled, so the integration now resolves the optional event API reflectively and logs an explicit error if a loaded JourneyMap version lacks it. This removes the hard build dependency while retaining the same runtime listener behavior when the API is present.
