...
- The latest merged `.\gradlew.bat build --no-daemon --no-problems-report` succeeded on 2026-08-16 with the recipe-free class-reset feature and all prior forward-marker retirement changes. It ran `ClassResetPolicyTest`, `FortChestTeamAccessTest`, and every Warday focused check. The artifact and sole refreshed distributable are both 303,564 bytes with SHA-256 `7F180EBB9BFD9CB456D3ED539921C468FBA8B985D5A2215DB021A6B52C2097B7`; the bundle contains no standalone `warday-1.0.0.jar`. Static JAR inspection confirmed the token classes/model and required Puffish Skills 0.18.3 dependency are packaged, while `data/fortifications/recipe/class_reset_token.json` is absent.
