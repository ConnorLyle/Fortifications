...
- The latest merged `.\gradlew.bat build --no-daemon --no-problems-report` succeeded on 2026-08-16 with the updated FortChest and animated Class Reset Token textures. It ran `ClassResetPolicyTest`, `FortChestTeamAccessTest`, and every Warday focused check. The artifact and sole refreshed distributable are both 304,995 bytes with SHA-256 `6AB03764C6FA96D6CD83B1F2EF66FB2FFC0D8D5CB083F6B480ADCEEC1157019F`; the bundle contains no standalone `warday-1.0.0.jar`. Static JAR inspection confirmed both new textures, the class gem's exact animation metadata, the token model, and required Puffish Skills 0.18.3 dependency are packaged, while `data/fortifications/recipe/class_reset_token.json` remains absent.
