...
- Animated Nexus texture provenance: the user supplied `dark's textures/nexustest/nexus.png` and `nexus.png.mcmeta` on 2026-08-15. The 16x32 two-frame PNG was copied byte-for-byte to `Warday-Mod/Warday/src/main/resources/assets/warday/textures/block/nexus.png` with SHA-256 `276742615D01BD466321DEC89F82FB04D4B6F5E825495861B95C52A5F1BF03CE`; the adjacent metadata displays frames 0 and 1 for 20 ticks each. Both resources are packaged in the Warday JAR. In-game animation and appearance still need runtime verification.
...
- Final Warday verification on 2026-08-15: `gradle build --no-daemon --no-problems-report` succeeded with all focused checks after adding `/warday clear` and two-pass entity cleanup verification to manual clearing and preparation. `WarDayAttackerTerrainPlanTest` covers the two-consecutive-empty-pass rule as well as the exact map/border, spawn-fallback, and arena-copy calculations. Warday still exempts team-shared FortChest contents from storage clearing while retaining its match-time storage-access ban. The build artifact and refreshed `MYTH MODS FOR DEREK/warday-1.0.0.jar` are both 190,432 bytes with SHA-256 `98CF0425B8BF5D6801D1745C8540EFE44F603B86042182525B6B8C4505382C62`.
