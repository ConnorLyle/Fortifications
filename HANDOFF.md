...
- Animated Nexus texture correction: the previously packaged 16x32 file contained two identical stripe frames, producing the broken striped block shown in-game. It was replaced byte-for-byte from `dark's textures/nexustest/nexus.png` with the intended 16x64 four-frame sheet at SHA-256 `0CA1889D0504C08FC3E3158AA5CCE5C2B0FCE0731586183E6F0299F795284C9A`; `nexus.png.mcmeta` now cycles frames 0-3 for 20 ticks each. Static JAR inspection verified both corrected resources are packaged. In-game animation and appearance still need runtime verification after a client restart or resource reload.
...
- Final Warday verification on 2026-08-15: `gradle build --no-daemon --no-problems-report` succeeded with all focused checks after adding `/warday clear`, two-pass entity cleanup verification, and the corrected four-frame Nexus animation at 20 ticks per frame. `WarDayAttackerTerrainPlanTest` covers the two-consecutive-empty-pass rule as well as the exact map/border, spawn-fallback, and arena-copy calculations. Warday still exempts team-shared FortChest contents from storage clearing while retaining its match-time storage-access ban. The build artifact and refreshed `MYTH MODS FOR DEREK/warday-1.0.0.jar` are both 191,331 bytes with SHA-256 `1148C16E43795A1065B02F5EB4FB53DB1A6B160E80219FCECA4B3D475D528829`.
...
- Preparation clears every non-player entity plus the full bounded target arena, copies surrounding blocks outside claimed chunks, copies the centered/rotated claim into the reserved columns, and transfers source biome cells. Each expensive block stage is cursor-driven and bounded per server tick; progress is runtime-only and a restart cancels the job. The target dimension must remain dedicated to War Day.
