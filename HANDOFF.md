...
- The Warday mod has a command surface under `/warday` in `WarDayCommands`:
  ...
  - `/warday prepare confirm` copies a bounded attacker terrain window and overlays the rotated defender claimed cluster in the configured War Day dimension.
...
- During respawn cooldown, participants automatically spectate an eligible living teammate in entity-camera POV. A synchronized client flag maps left/right click to validated previous/next teammate requests, while no-target, target-loss, reconnect/restart, fanfare, and cleanup paths restore or reselect the camera safely.
...
- The `warday_roster` sidebar now shows a right-aligned, color-coded live-health/status column: half-heart current/maximum health for online players, `RESP Ns` during persisted respawn cooldowns, and `OFF` for disconnected participants.
...
- The rule now triggers on the configurable 15th successful break in an inclusive rolling 30-second window and applies an exact configurable 25% `BLOCK_BREAK_SPEED` reduction plus Glowing for 60 seconds by default. It uses one named transient modifier, so retriggers refresh rather than stack.
...
- `WarDayRapidBreakRuleTest` covers the 15th-break boundary, rolling-window expiry, retrigger reset, and exact percentage calculation. `gradle build --no-daemon --no-problems-report` succeeded with all five focused checks. The artifact and refreshed `MYTH MODS FOR DEREK/warday-1.0.0.jar` are both 144,914 bytes with SHA-256 `07E79B3689F7E4E800A2D9178EDE79ADDEAAEDA072FB64650A01924E728B5663`.
...
- `.\gradlew.bat build --no-daemon --no-problems-report` succeeded on 2026-08-07. Static JAR inspection confirmed both the override and metadata ordering. The artifact and refreshed `MYTH MODS FOR DEREK/fortifications-1.0.0.jar` are both 78,638 bytes with SHA-256 `9E7137E7ABA02BD5009005D3A2E6056274A4F378DE3700FD092F96116D4525EB`.
...
- Attacker terrain now uses a configurable square radius centered on its spawn marker: radius 8 by default selects 289 chunks, with a separate default 289-chunk safety guardrail and a hard radius bound of 12. Copying is clipped to the configured map border independently of source chunk alignment or coordinates.
- `/warday prepare` reports selected source bounds, target footprint, clipped columns, rotation, and loaded/unloaded source chunks. Confirm clears the full bounded arena for repeatability, copies the attacker background first, clears the defender mapped footprint, and applies the rotated defender base last so both solid blocks and intentional air win overlaps.
- Expanded copy/conflict scans and full-arena clearing skip all-air chunk sections. `WarDayAttackerTerrainPlanTest` covers radius counts, negative chunks, translated anchors, exact border semantics, and clipping. `gradle build --no-daemon --no-problems-report` succeeded on 2026-08-07 with all focused checks. The build artifact and refreshed distributable are both 136,653 bytes with SHA-256 `B6C6706873A33C1780F4D7D98B80F2E710AB7F897FDAB5F4C479A7E10D46AC67`.
...
- A clientbound active-state payload gates left/right click interception to Warday cooldowns only, while a serverbound cycle payload requests previous/next selection. Every request is revalidated server-side, preventing opponent or arbitrary-entity cameras. Camera/input state is disabled before cooldown restoration, fanfare, exact match restoration, and `/warday end`; login/restart reconstruction selects a fresh runtime target from persisted team and cooldown state.
- `WarDaySpectatorCycleTest` covers empty, single, and multiple candidates, initial selection, forward/backward cycling, wraparound, and missing-target recovery. Both focused test executables run under Gradle `check`; `gradle build --no-daemon --no-problems-report` succeeded on 2026-08-07. The build artifact and refreshed `MYTH MODS FOR DEREK/warday-1.0.0.jar` are both 132,628 bytes with SHA-256 `E542475F51515CA8BDC7AE87820A41E42212E6F3166A1DBC46E1EF0A550AA6BE`.
...
- Player-line health uses Minecraft 1.21.1's per-score `FixedFormat`, leaving the integer score available for stable roster ordering. Online values follow vanilla half-heart rounding, respect modded maximum health, and are colored by health percentage; absorption is excluded. Persisted pending respawns render a countdown and disconnected participants render `OFF`.
- Sidebar synchronization remains once per second and now reuses stable score holders instead of clearing the objective whenever health or status changes. It removes only stale holders, and the existing page rotation, restart reconstruction, previous-objective restoration, and match cleanup paths remain intact.
- `WarDayRosterHealthTest` covers rounding, clamping, invalid/boosted health, color thresholds, offline status, and respawn countdown boundaries and runs under Gradle `check`. `gradle build --no-daemon --no-problems-report` succeeded on 2026-08-07. The build artifact and refreshed `MYTH MODS FOR DEREK/warday-1.0.0.jar` are both 123,341 bytes with SHA-256 `AB4E9BF9F0D5ED5983E77EAC312B31647BF8CF0172B139094D4DABC7022AF28B`.
...
- The worktree contains the uncommitted live-health, respawn-spectating, attacker-terrain, skill-tree-removal, JourneyMap privacy, and rapid-break implementations, their automated tests/resources, networking/client-control additions, documentation updates, and both refreshed distributables. The repository-level `mods` directory also holds untracked local compile-only dependencies restored from the active CurseForge instance. Preserve them and inspect `git diff` before further edits.
...
3. Manually verify the expanded attacker terrain preparation matrix recorded under the Arena preparation item, especially route coverage, defender overlap priority, border clipping, repeat prepare after radius changes, and preparation responsiveness.
4. Verify only the Classes Puffish tree remains, then run the JourneyMap same-team/opponent/team-change/optional-mod matrix and the rapid-break threshold/exemption/cleanup matrix from `WARDAY_TODO.md`.
5. Manually verify respawn teammate POV and both mouse-cycle directions with multiple teammates and an opponent, including target loss, no-target recovery, reconnect/restart, cooldown completion, fanfare, cleanup, ordinary spectators, and a second match.
6. Manually verify the sidebar layout, live health/status column, damage/healing updates, modded maximum health, respawn countdown, disconnect/reconnect, team membership, page rotation, offline-name fallback, restart reconstruction, cleanup, and restoration of a pre-existing sidebar objective.
7. Manually verify the boss bar during start/countdown, death/respawn, logout/reconnect, server restart, defender timeout, nexus victory, `/warday end`, fanfare transition, and a second match. Confirm there are no stale or duplicate HUD elements.
8. Run the entity carryover manual matrix from the first `WARDAY_TODO.md` item, including tamed wolves, passenger trees, leashes, all rotations, an unclaimed-hole entity, the configured cap, repeated matches, and restart cleanup.
9. Record successful runtime results beneath each applicable queue item before striking it through; keep any item with a failure or untested edge case unchecked.
...
- Defender guardrails check connected cluster size and footprint width/depth. Attacker terrain has its own configurable square radius and chunk-count guardrail.
...
- Preparation clears the full bounded target arena for stale-free repeated copies, copies the clipped unrotated attacker terrain background, then clears and overlays the rotated defender claim shape. The target dimension must remain dedicated to War Day.
