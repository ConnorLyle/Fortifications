...
- Terrain preparation copies the exact nexus-centered 256x256 source square, rotates the complete square toward west, reserves claimed chunks out of the surrounding pass, and copies the claim into those same columns. Loading, four-corner validation, checks, clearing, copying, and source-biome transfer are resumable server-tick phases with a 10 ms budget. No attacker marker or remote biome search is used.
...
- Texture provenance: the user supplied `C:/Users/lylec/Downloads/chest.png` on 2026-08-15. It was copied byte-for-byte to `assets/fortifications/textures/entity/chest/fort_chest.png`; both source and project texture are 64x64 RGBA with SHA-256 `E39AD381FB0ED436BBB30EA4E395F12E858E0FFE4861B82094129C071E046B67`.
- `./gradlew.bat build --no-daemon --no-problems-report` succeeded on 2026-08-15 with the focused `FortChestTeamAccessTest`; packaged metadata contains required Architectury/FTB Library/FTB Teams dependencies and the supplied texture. The artifact and refreshed `MYTH MODS FOR DEREK/fortifications-1.0.0.jar` are both 102,654 bytes with SHA-256 `56B9AC08B0584FD79FF2C6502575AAC62CE9A57259C12CCF011F489C7F8D3E6C`.
...
- Final Warday verification on 2026-08-15: `gradle build --no-daemon --no-problems-report --no-build-cache` succeeded with all focused checks after the arena became a nexus-centered 256x256 source copy with four attacker corner spawns and a respawn picker. Warday now exempts team-shared FortChest contents from copy/start cleanup while retaining its match-time storage-access ban. The build artifact and refreshed `MYTH MODS FOR DEREK/warday-1.0.0.jar` are both 177,914 bytes with SHA-256 `B04B36AAA866478C4DABC96FDBA9892EABCF5F1AB77A2C6F11089881ADE9A243`.
...
- Several gameplay values are effectively hardcoded:
  ...
  - All four attacker targets use a 16-block border inset; corner safety search is limited to an 8-block horizontal radius.
