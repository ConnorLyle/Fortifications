...
- [ ] Add a boss-bar match timer at the top of the screen and keep it synchronized with the authoritative match clock.
  ...
  - Verification: boundary calculations passed for match start, sub-second countdown, the final two seconds, expiry, and post-expiry clamping. `gradle build --no-daemon --no-problems-report` succeeded on 2026-08-06. The build artifact and refreshed `MYTH MODS FOR DEREK/warday-1.0.0.jar` are both 111,385 bytes with SHA-256 `5FFBF88DC42834CF97CCB94D8F33962FF26EA383655A06D49272A27992399488`. No project test sources currently exist.
...
- [ ] Add a sidebar scoreboard that lists Team 1 and Team 2 and their participating players.
  - Implementation added 2026-08-06: Warday creates a server-owned `warday_roster` sidebar objective with blank numeric formatting, colored team headers, and alphabetically sorted names from the persisted defender/attacker participant UUID sets. Online names are preferred, cached offline profiles are supported, and an eight-character UUID prefix is the safe fallback when no profile name is available.
  - Minecraft's 15-line sidebar limit is handled with symmetrical pages of up to six players per team; both team headers remain visible and pages rotate every five seconds. The match-start state persists the previously displayed sidebar objective, which is restored at match cleanup if Warday still owns the display slot. If another system takes the slot mid-match, Warday records that newer objective before reclaiming the slot so cleanup restores the latest external owner.
  - Verification: pagination calculations covered every defender/attacker size combination from 0 through 30, never exceeded 14 rendered lines, and showed every participant exactly once per page rotation. `gradle build --no-daemon --no-problems-report` succeeded on 2026-08-06. The build artifact and refreshed `MYTH MODS FOR DEREK/warday-1.0.0.jar` are both 116,294 bytes with SHA-256 `C5AEEBC0A78E339416520390263C884BE80AE9BE3EADA53751B94392DC59CD9A`. No project test sources currently exist.
